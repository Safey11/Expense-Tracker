...
export default function Expense() {
  ...
  const [editedExpense, setEditedExpense] = useState({}); // Store edited values
  const [isSidebarOpen, setIsSidebarOpen] = useState(false); // State for sidebar toggle

  useEffect(() => {
    if (status === "unauthenticated") router.push("/login");
    else if (status === "authenticated") {
      fetch(`/api/expenses?userEmail=${session?.user?.email}`)
        .then((res) => res.json())
...
const startEditing = (expense) => {
  ...
  setEditedExpense({
    ...expense,
    date: formatDateForInput(expense.date) || new Date().toISOString().split("T")[0]
  });
};
...
const formatDateForInput = (dateString) => {
  if (!dateString) return new Date().toISOString().split("T")[0]; // Default to today's date
  const date = new Date(dateString);
  if (isNaN(date.getTime())) return new Date().toISOString().split("T")[0]; // If invalid, return today's date
